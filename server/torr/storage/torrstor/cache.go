...
func (c *Cache) cleanPieces() {
	...
	// Защита от параллельной очистки
	if !c.muRemove.TryLock() {
		return // Уже выполняется очистка в другой горутине
	}
...
func (c *Cache) getRemPieces() []*Piece {
	// Копируем читателей без долгой блокировки
	c.muReaders.Lock()
	...
	// Собираем диапазоны чтения от активных читателей
	ranges := make([]Range, 0)
	...
	// Определяем, какие куски можно удалить
	for id, p := range c.pieces {
		...
		if len(ranges) > 0 {
			...
			// При прелоаде чистим все, кроме начала и конца файла
			if p.Size > 0 && !c.isIdInFileBE(ranges, id) {
				piecesRemove = append(piecesRemove, p)
			}
		}
